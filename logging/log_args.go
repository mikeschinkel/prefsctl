...
func appendLogArgs(args ...any) (_ []any, err error) {
	...
	for i < len(args) {
		...
		if i == len(args)-1 {
			...
			err = errors.Join(ErrNotEnoughArgsPassed, fmt.Errorf("%s=%s", CallerLogArg, funcName))
			packageSlog.Debug("not enough parameters", CallerLogArg, funcName, ErrorLogArg, err)
	...
	err = errs.Err()
	if err != nil {
		packageSlog.Error("missing label or value passed to AppendLogArgs", ErrorLogArg, err)
	}
	return newArgs, err
}
