...
// MustKeepGroup returns true is a value matches ANY Keep filters. Rarely
...
// OmitGroup returns true if there is a match for ANY Omit filters.
...
//
//// KeepKeyValue1 returns true is a value matches ANY Keep filters. Rarely
//// use this as it overrides all Omit filters.
//func KeepKeyValue1(ff []Filter, keyValue Code, targets ...Target) (keep bool, err error) {
//	return KeepGroup(ff, keyValue, targets...)
//}
//
//// OmitKeyValue1 returns true if there is a match for ANY Omit filters.
//func OmitKeyValue1(ff []Filter, keyValue Code, targets ...Target) (keep bool, err error) {
//	return KeepGroup(ff, keyValue, targets...)
//}

// MustKeepKeyValue2 returns true is a key or value as appropriate matches ANY Keep filters.
...
// OmitKeyValue2 returns true is a key or value as appropriate matches ANY Omit filters.
...
func matchValueFilter(filter Filter, value string) (match bool, err error) {
	...
	case Contains, Prefix, Suffix, Entirety, Regexp:
		...
		if !ok {
			err = errors.Join(ErrFailedToAccessGroupKeyValues,
				fmt.Errorf("%s=filters.ValuesGetter", logging.TypeExpectedLogArg),
				err,
			)
			goto end
		}
				...
				case Regexp:
					...
					if err != nil {
						err = errors.Join(ErrInvalidRegExp,
							fmt.Errorf("%s=%s", logging.RegexpLogArg, reString),
							err,
						)
						goto end
					}
	...
	case Func:
		...
		if !ok {
			err = errors.Join(ErrFailedTypeAssertion,
				fmt.Errorf("%s=filters.Funcs1Getter", logging.TypeExpectedLogArg),
				err,
			)
			goto end
		}
	...
	case InvalidMatches:
		...
		err = errors.Join(ErrInvalidFilterMatchCriteria,
			fmt.Errorf("%s=%s", logging.TypeExpectedLogArg, matchCriteria.String()),
			err,
		)
		goto end
	}
...
func matchKeyValueFilter(filter Filter, key Code, value string) (match bool, err error) {
	...
	if !ok {
		err = errors.Join(ErrFailedTypeAssertion,
			fmt.Errorf("%s=filters.Funcs2Getter", logging.TypeExpectedLogArg),
			err,
		)
	}
